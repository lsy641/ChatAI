...
def room(request,room_id):
    ...
    if int(request.COOKIES.get("login_room",-1))==room_id:
        ...
        if hit.speaker_name is None:
            #rep = redirect(reverse("chatai:login", kwargs={"room_id": room_id}))
            #rep = redirect(reverse("chatai:room", kwargs={"hit": hit, "conv": conv, "room_id": room_id, "name": name}))
            ...
            if name==hit.speaker_name:
                ...
                if not request.POST.get("choice",False):
                    #rep = redirect(reverse("chatai:room", kwargs={"hit":hit,"conv":conv,"room_id":room_id,"name":name,'error_message':"You didn't select a choice."}))
                    ...
                    if request.POST.get("context",False):
                        ...
                        conv.save()
                        rep = redirect(reverse("chatai:room", kwargs={"room_id": room_id}))
                        #rep = redirect(reverse("chatai:room", kwargs={"hit": hit, "conv": conv, "room_id": room_id, "name": name,"cur_turn":0}))
                        #rep = render(request, 'chatai/room.html',{"hit": hit, "conv": conv, "room_id": room_id, "name": name,"cur_turn":0})
                    else:
                        #rep = redirect(reverse("chatai:room", kwargs={"hit": hit, "conv": conv, "room_id": room_id, "name": name,'error_message': "你选择创造场景但没有输入文本"}))
            ...
            if request.POST.get("speaker_text",False) and cur_turn==0:
                ...
                conv.save()
                rep = redirect(reverse("chatai:room", kwargs={"room_id": room_id}))
                #rep = redirect(reverse("chatai:room",kwargs={"hit": hit, "conv": conv, "room_id": room_id, "name": name,"history":conv.utterances.order_by("send_time"),"cur_turn":int(1-cur_turn),"length":conv.utterances.count()}))
                ...
                #rep = redirect(reverse("chatai:room", kwargs={"hit": hit, "conv": conv, "room_id": room_id, "name": name,"history":conv.utterances.order_by("send_time"),"cur_turn":cur_turn,"length":conv.utterances.count()}))
...
def close(request,room_id):
    ...
    name = request.COOKIES.get("name", None)
    try:
        hit = Hit.objects.get(hit_number=room_id)
    except (KeyError, Hit.DoesNotExist):
